...
describe("nativeTitle", () => {
  it("uses the default title without a useful session title", () => {
    expect(nativeTitle(null)).toBe("Genix Kilo Code")
    expect(nativeTitle(session(""))).toBe("Genix Kilo Code")
    expect(nativeTitle(session("New session - 2026-05-06T10:39:00.000Z"))).toBe("Genix Kilo Code")
  })
